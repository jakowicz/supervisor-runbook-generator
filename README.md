...
| `D0001`, `D0002`, … | **Dispatcher** | F014 creates `D0001`; each earlier D dispatcher creates at most one successor | `projects/<slug>/authoring-runbooks/` | **Authoring coordination only.** A D task expands one eligible planning chapter, allocates the next bounded B batches, and leaves one successor dispatcher only if more catalogue work remains. It does not build the product or write R implementation files itself. |
