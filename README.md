...
| `B0001`, `B0002`, … | **Bounded implementation-runbook authoring batch** | The initial F014 output, then later D dispatchers | `projects/<slug>/authoring-runbooks/` | **Runbook-writing tasks.** F014 creates the first B batches; D dispatchers create later batches. A B task writes the next small set of R files. Like GB, it has a hard limit: one B task may write no more than five R files. |
