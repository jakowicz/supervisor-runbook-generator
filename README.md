...
| `GB0001`, `GB0002`, … | **Bounded game-design authoring batch** | The G-series dispatchers, for games only | `projects/<slug>/game-design-runbooks/` | **Game-design runbook-writing tasks.** A GB task writes the next small set of G design runbooks. Like B, it has a hard limit: one GB task may write no more than five G files. |
